...
/**
 * Tests for index.astro (homepage)
 *
...
describe('index.astro (homepage)', () => {
	...
	describe('section order', () => {
		it('has Hero before Features', () => {
			const heroIndex = pageContent.indexOf('<Hero1')
			const feature1Index = pageContent.indexOf('<Feature1')
			expect(heroIndex).toBeLessThan(feature1Index)
		})

		it('has Features in correct order', () => {
			const feature1Index = pageContent.indexOf('<Feature1')
			const feature2Index = pageContent.indexOf('<Feature2')
			const feature3Index = pageContent.indexOf('<Feature3')
			expect(feature1Index).toBeLessThan(feature2Index)
			expect(feature2Index).toBeLessThan(feature3Index)
		})

		it('has Features before Stats', () => {
			const feature3Index = pageContent.indexOf('<Feature3')
			const statsIndex = pageContent.indexOf('<Stats1')
			expect(feature3Index).toBeLessThan(statsIndex)
		})

		it('has Stats before Testimonial', () => {
			const statsIndex = pageContent.indexOf('<Stats1')
			const testimonialIndex = pageContent.indexOf('<Testimonial1')
			expect(statsIndex).toBeLessThan(testimonialIndex)
		})

		it('has Testimonial before Pricing', () => {
			const testimonialIndex = pageContent.indexOf('<Testimonial1')
			const pricingIndex = pageContent.indexOf('<Pricing1')
			expect(testimonialIndex).toBeLessThan(pricingIndex)
		})

		it('has Pricing before FAQ', () => {
			const pricingIndex = pageContent.indexOf('<Pricing1')
			const faqIndex = pageContent.indexOf('<Faq1')
			expect(pricingIndex).toBeLessThan(faqIndex)
		})

		it('has FAQ before CTA', () => {
			const faqIndex = pageContent.indexOf('<Faq1')
			const ctaIndex = pageContent.indexOf('<Cta1')
			expect(faqIndex).toBeLessThan(ctaIndex)
		})
	})
	...
	describe('content structure', () => {
		it('has hero with label, headline, and buttons', () => {
			expect(pageContent).toContain("label: 'NextNode Solutions'")
			expect(pageContent).toContain("headline: 'Build faster")
		})

		it('has feature items with id, title, description', () => {
			expect(pageContent).toContain("id: 'feature1-deploy'")
			expect(pageContent).toContain("title: 'Deploy in seconds'")
		})

		it('has stats with target numbers', () => {
			expect(pageContent).toContain('target: 50')
			expect(pageContent).toContain('target: 99')
		})
		...
		it('has FAQ items with questions and answers', () => {
			expect(pageContent).toContain("question: 'How does billing work?'")
